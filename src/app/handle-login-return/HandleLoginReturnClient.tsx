...
import type { KratosUser } from '../util/types/kratos-user.type';

const KRATOS_URL = process.env.NEXT_PUBLIC_KRATOS_URL!;
const RETURN_TO  = process.env.NEXT_PUBLIC_KRATOS_RETURN_TO!;

export default function HandleLoginReturnClient() {
  const searchParams = useSearchParams();

  // Read params in render (recommended with app router)
  ...
  useEffect(() => {
    if (!KRATOS_URL) {
      setError('Missing NEXT_PUBLIC_KRATOS_URL');
      setLoading(false);
      return;
    }
    let retries = 0;
      ...
      try {
        ...
        if (res.status === 401) {
          if (retries < maxRetries) {
            retries++;
            retryTimer.current = setTimeout(fetchWhoami, 400 + retries * 200); // small backoff
            return;
          }
    ...
    // First attempt—slightly delayed if cookie likely exists
    const initialDelay = document.cookie.includes('ory_kratos_session') ? 150 : 400;
    retryTimer.current = setTimeout(fetchWhoami, initialDelay);
